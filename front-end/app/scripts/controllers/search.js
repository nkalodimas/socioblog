'use strict';

app.controller('SearchCtrl', function ($scope, $routeParams, Post){
    $scope.posts = [];
    $scope.previewChars = 5;

    var pageNum = $routeParams.pageNumber || 1;

    $scope.currentPage = pageNum;
    
    $scope.searchTerm = $routeParams.searchTerm;

    $scope.search = function(pageNum) {
        Post.search({searchTerm:$scope.searchTerm, pageNumber:pageNum}, function(data) {
            $scope.posts = data.posts;
            $scope.totalPages = data.totalPages;
            $scope.totalPosts = data.totalPosts;
        });
    }
    
    $scope.showNewerPosts = function (){
        return $scope.currentPage > 1;
    };
    
    $scope.showOlderPosts = function (){
        return $scope.currentPage < $scope.totalPages;
    };
    
    $scope.showNewerPostsUrl = function (){
        return "#/posts/search/" + $scope.searchTerm + "/" + ($scope.currentPage - 1);
    };
    
    $scope.showOlderPostsUrl = function (){
        return "#/posts/search/" + $scope.searchTerm + "/" + ($scope.currentPage + 1);
    };
    
    $scope.search(pageNum);

  });